...
async function createPadroes(adicionarUsuario, adicionarProduto, relatorios, reporEstoque, gerenciarPermissoes, adicionarPromocao, cadastrarEmpresa, prestesAVencer, iniciarExpediente, adicionarCliente) {
    ...
    const infoUser = [adicionarUsuario, adicionarProduto, relatorios, reporEstoque, gerenciarPermissoes, adicionarPromocao, cadastrarEmpresa, prestesAVencer, iniciarExpediente, adicionarCliente];
...
async function listPadroes() {
    const sql = "SELECT * FROM padroes p WHERE ativo = 1 ORDER BY p.id DESC LIMIT 1";
